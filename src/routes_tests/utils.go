...
func CreateTestTransaction(description string, amount decimal.Decimal, date time.Time, userID uint, categoryID uint) *models.Transaction {
	transaction, err := repository.CreateTransaction(description, amount, date, userID, categoryID)
	if err != nil {
